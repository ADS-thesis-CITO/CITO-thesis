'''
:Author: Tom Klopper. 
:Goal: To transform unprocessed answers into cleaned single characters for further analysis for all threshold values. 
...
# Define Functions. 
def RemoveLine(img_gray, thresh_img, threshold_value): 
    '''
    ...
    ret, thresh_img = cv2.threshold(img_gray, threshold_value, 255, cv2.THRESH_BINARY)
...
def CreateImageCharacters(thresh_img, all_rectangles, name, img_size, test_name, threshold_value): 
    '''
    ...
    for idx, rect_key in enumerate(all_rectangles.keys()):
        ...
        if w > 0.15 * nw or h > 0.15*nh: # Filter out noise. 
            ...
            try: 
                ...
                continue
            cv2.imwrite(f"C://Users//tklop//ThesisADS//CharacterImages//threshold_{threshold_value}//{img_size}x{img_size}//{test_name}_{name[:-4]}_{idx}.jpg", resized) # Make name more general, i.e.: answer1_1 
            


# Run the code. 
if __name__ == "__main__":
    # Iterate through multiple threshold values. 
    threshold_value_list = [200,205,210,215,220,225,230,235,240,245]
    for threshold_value in threshold_value_list:
        ...
        for idx_test, test_name in enumerate(test_names): 
            ...
            for idx_names, name in enumerate(img_names): 
                ...
                img = RemoveLine(img_gray, thresh_img, threshold_value)
                ...
                for img_size in img_sizes: 
                    CreateImageCharacters(img, nested_rectangles, name, img_size, test_name, threshold_value)
